...
### 5.2 Phase 2: ToolScheduler 适配 (Week 3)

```python
# 适配 ToolScheduler 到 LangChain Tool
class LangChainToolAdapter:
    """将 LangChain Tool 适配到 ToolScheduler"""
    ...
    async def execute_dag(self, tool_calls: List[ToolCall]) -> ToolBatchResult:
        scheduler = ToolScheduler()
        return await scheduler.execute_batch(tool_calls, self._registry)
```

### 5.3 Phase 3: 节点实现 (Week 4-5)

```python
# graph/nodes.py
async def coder_node(state: CodingAgentState) -> dict:
    """Coder 节点 - 支持 DAG 工具调用"""
    ...
    tool_calls = parse_tool_calls_from_llm(result)
    if has_dag_dependencies(tool_calls):
        adapter = LangChainToolAdapter(get_coder_tools())
        tool_result = await adapter.execute_dag(tool_calls)
        # 处理结果...
