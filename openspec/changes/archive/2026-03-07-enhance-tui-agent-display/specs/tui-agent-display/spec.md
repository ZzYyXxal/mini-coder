...
子代理可能输出**非工具类结论**（例如 Reviewer 的 Pass/Reject），与工具调用区分展示，避免与 `[Tool]` 行混淆。

- **工具行**：`↳ [Tool] Read: path`、`↳ [Tool] Grep: ...`，表示当前子代理调用的工具。
- **结论行**：如 `↳ [Pass] 代码质量检查通过`、`↳ [Reject] ...`，可单独标注为「结论」或使用不同样式（如 `[dim]`），不与 `[Tool]` 混用同一标签。
