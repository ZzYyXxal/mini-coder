# TUI Binary Capability

Provides a standalone binary executable for mini-coder TUI, compiled from Python source.

## Purpose

Enable users to run mini-coder TUI without requiring Python runtime or external dependencies. The binary is self-contained and can be executed directly from the terminal on any supported platform.

## Implementation Status

**Status:** Implemented - Build pipeline complete with PyInstaller integration.
...
- **WHEN** binary is compiled with optimization flags
...
The system SHALL provide a configurable entry point that initializes TUI application, sets up logging, and handles command-line arguments. The entry point SHALL gracefully handle initialization failures with user-friendly error messages.
...
- Build infrastructure implemented in `scripts/build-tui.sh`
- PyInstaller spec files generated for each platform
- Makefile provides convenient build targets
- Version information available via `--version` CLI flag and TUI about menu
